Function Add-SATPClaimrule {
  ...
  PARAM (
    [PARAMETER(Mandatory=$True, Position=0,HelpMessage = "VMHost",ParameterSetName='Default', ValueFromPipeline=$true)][PSCustomObject[]]$VMHost,
  ...
  process {
    ...
    foreach ($CurrentVMHost in $VMHost) {
		$HostName = $CurrentVMHost.Name
		$esxcli = Get-EsxCli -VMHost $CurrentVMHost -V2 -ErrorAction "SilentlyContinue"
		if (-not $esxcli) {
		  write-host "Add-SATPClaimrule ($HostName): Can not get ESX CLI for host - skipping SATP claimrule creation" -foreground 'red'
		}
		else {
		  # creating rules list
		  $RulesToAdd = @()
